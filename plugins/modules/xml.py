...
description:
- A customized version of community.general.xml dedicated to AMQ configuration xpath/xslt.
options:
  path:
    description:
...
def main():
    ...
    # Check if we have lxml 3.0.0 or newer installed
    if not HAS_LXML:
        module.fail_json(msg=missing_required_lib("lxml"), exception=LXML_IMP_ERR)
    elif etree.LXML_VERSION[0] < 3:
        module.fail_json(msg='The xml ansible module requires lxml 3.0.0 or newer installed on the managed machine')
